...
namespace Neo.Network.P2P
{
    internal class TaskManager : UntypedActor
    {
        ...
        private void RequestTasks(TaskSession session)
        {
            ...
            if ((!HasHeaderTask || globalTasks[HeaderTaskHash] < MaxConncurrentTasks) && blockchain.HeaderHeight < session.LastBlockIndex)
            {
            ...
            else if (blockchain.Height < session.LastBlockIndex)
            {
            ...
            else if (blockchain.HeaderHeight >= session.LastBlockIndex
                    && TimeProvider.Current.UtcNow.ToTimestamp() - PingCoolingOffPeriod >= blockchain.GetBlock(blockchain.CurrentHeaderHash)?.Timestamp)
            {
                session.RemoteNode.Tell(Message.Create(MessageCommand.Ping, PingPayload.Create(blockchain.Height)));
            }
        }
    }
